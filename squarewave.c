...
volatile uint16_t num_roll = 0;
volatile uint8_t num_match = 0;
volatile uint8_t silent = 0;
...
{
  cli();
  num_roll = notes[midi_pitch-21][0];
  num_match = (uint8_t)notes[midi_pitch-21][1];
  OCR2B = num_match;
  overflows = 0;
  silent = 0;
  sei();
